...
| 2-b | How is the `neural` data processed? | 🟢🔴🟢 🟡🔵🔵 | 🟢🔴🟡 🟡🔴🟡 | 🔴🔴🔴 🔴🔴🔴 | Only one agent produced the "really" correct solution — since the experiment timestamps are already aligned, there is no need to resample. Two agents made some very wrong choices on time bin size. |  | `RESAMP=2?`, `TIMERES=2` |
...
| 2-d | How is the `neural` data temporally binned/resampled? | 🟢🔴🔵 🟡🔵🔵 | 🟡🔴🟡 🟡🔴🟡 | 🟢🔴🔴 🔴🔴🔴 | The data is ~11 Hz, so some agents resampling to a 30 ms time bin is ok, but in principle no resampling is needed. Two agents made some really bad choices (2-b). |  | `TIMERES` |
...
| 6-a | What variables in the raw data is `output` *Image change* derived from? | 🔵🔵🟢 🔵🔵🔵 | 🔵🔵🔵 🔵🟡🟡 | 🔴🔵🟡 🔴🔴🟡 | Some solutions included the grey screen as an extra image category. |  | `SEMANTIC` |
