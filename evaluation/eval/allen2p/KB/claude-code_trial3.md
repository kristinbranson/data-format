...
**Note:** manual uses all VisualBehavior project code experiments, while this looks for specific session_types that correspond to active sessions, these don't seem to match
...
**Note:** not comparing filtering as that is in the next question
...
**Note:** manual uses trial table, but agent uses stimulus presentation intervals
...
**Note:** multiple passes over nwb files sounds problematic
...
**Note:** not ideal to open each nwb file 3 times
