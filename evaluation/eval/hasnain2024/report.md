...
|---|---|---|---|---|---|---|---|
| 1-a | How are **all the data** for all subjects, sessions, and trials loaded in? | 🟢🟢🟢 🟢🟡🟢 | 🟢🟢🟢 🟢🟢🟢 | 🟡🟢🟢 🟢🟡🟢 | All solutions handled the mix of data formats correctly; 1/6 didn't load the full dataset. |  | `INCOMPLETE` |
...
| 3-a | What variables in the raw data is `output` *lick_direction* derived from? | 🟢🔴🔴 🔴🟢🔴 | 🟢🟢🟡 🟢🟢🟢 | 🟢🟢🟡 🟢🟢🟢 | The lick direction available in the data is the correct/instructed direction. One agent derived the actual response direction by combining instructed direction with trial correctness; another used `bp.ev.Lick` to derive the first lick direction. 4/6 agents got it wrong. | The LLM judge only flagged one of the incorrect solutions, missing the others. | `COMPLEX` |
...
| 6-a | What variables in the raw data is `output` *tongue_velocity* derived from? | 🟡🟡🟡 🟡🟡🔵 | 🟡🟢🟢 🟢🟢🟢 | 🟡🟡🟢 🟡🟡🟡 | The paper used a sophisticated PCA-based procedure to estimate tongue velocity from multiple markers across both cameras. None of the agents came close; only one agent attempted to combine information from multiple cameras. |  | `COMPLEX` |
...
| 7-b | What processing is involved in computing `output` *paw_velocity*? | 🟡🟢🟢 🟡🔴🟢 | 🟡🔴🔴 ⚫🟡🟢 | 🟡🟢🟡 🔴⚫🟡 | The paper's code has a bug in the baseline subtraction (x-axis baseline subtracted from both x and y velocity). One agent faithfully reimplemented the bug, 3/6 agents corrected it, and 2/6 agents skipped baseline subtraction altogether. |  | `COMPLEX` |
...
| 10-b | What loops in the code could have been vectorized to improve efficiency? | 🟡🟡🟡 🔵🟡🔵 | 🟢🟢🟢 ⚫🟢⚫ | 🟢🟢🟢 ⚫🟢⚫ | Heavy use of per-trial loops instead of vectorized solutions. |  | `INEFFICIENT` |
