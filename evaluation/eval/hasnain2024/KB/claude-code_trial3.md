...
**Note:** 10 ms bin width instead of 5 ms,  manual interprets params.smooth to be in ms, while agent interprets this as bins
...
**Code** (convert_data.py:729):
```python
hit_valid = hit[valid_trial_indices]
```

**What this does:** No output-specific filtering beyond the per-trial validity mask.
...
**Note:** smoothing sigma is much larger
