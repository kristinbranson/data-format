...
**Note:** agent also checks ft_move
...
**Note:** agent selects discriminating neurons based on d'
...
**Note:** manual makes trials all the same length, which was not necessary for the data spec. this requires padding the
...
**Note:** agent uses all unique categories, while manual groups by texture type
