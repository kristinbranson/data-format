...
**Note:** loads from three pqt manifest files
...
**Note:** subject comes from directory structure
...
**Note:** does not check covered(wheel_times, stim_on) & covered(camera_times, stim_on)
...
**Note:** agent uses binning function rather than just dividing by rounding to the nearest bin, which is more efficient since bins are evenly spaced
...
**Note:** uses brainbox's interpolate_position which hopefully does this right
...
**Note:** spike binning can be avoided by just rounding to the nearest 20ms
