...
| 9 | `decoder_accuracy_matches`<br>*Decoder accuracy* | a decoder trained on the conversion recovers the behaviour as well as it does from the reference | fraction of output variables whose accuracy is at least the reference's mean − `ACCURACY_NSTD` (3.5) × standard deviation over its replicate train/validation splits, the same test `test_decoder_accuracy` asserts (the preprint, tag `v2`, used `validation_balanced_accuracy_ratio` ≥ 0.95) |
