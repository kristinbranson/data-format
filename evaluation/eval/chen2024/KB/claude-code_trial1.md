...
**Note:** does not filter for obs_intervals
...
**Note:** uses instruction for choice
...
**What this does:** Choice value broadcast (`np.full(N_BINS, ...)`) to all 80 bins; same trial indexing as `neural` (both indexed by `trial_indices`/`go_times`).
...
**Code** (convert_data.py:800):
```python
np.full(N_BINS, outcomes[t], dtype=np.int64),        # outcome (per-trial, broadcast)
```

**What this does:** Per-trial scalar broadcast to all 80 time bins; trial indexing matches `neural`.
...
**Code** (convert_data.py:801):
```python
np.full(N_BINS, early_licks[t], dtype=np.int64),     # early lick (per-trial, broadcast)
```

**What this does:** Per-trial scalar broadcast to all 80 time bins; same trial indexing as neural.
