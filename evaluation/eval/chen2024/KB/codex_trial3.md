...
**Notes excerpt** (CONVERSION_NOTES.md / README.md):
> "Output is constant across time bins for a trial" (CONVERSION_NOTES.md:212)

**Code** (convert_data.py:441-452):
```python
...
**What this does:** The scalar choice code is broadcast via `np.full(N_BINS, ...)` so it shares the same 80-bin go-aligned axis as the neural matrix.
...
**Notes excerpt** (CONVERSION_NOTES.md / README.md):
> "Constant across time bins" (CONVERSION_NOTES.md:213)

**Code** (convert_data.py:443-450):
```python
        output_trial = np.vstack(
            [
                np.full(N_BINS, choice_code[trial_idx], dtype=np.int16),
...
**What this does:** Outcome scalar is broadcast across all 80 go-aligned bins per trial.
...
**Notes excerpt** (CONVERSION_NOTES.md / README.md):
> "Constant across time bins" (CONVERSION_NOTES.md:214)
...
**What this does:** Scalar early-lick code broadcast to all 80 go-aligned bins.
