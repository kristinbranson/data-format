...
- The data in directory `data` is from the paper "Separating cognitive and motor processes in the behaving mouse". 
...
- Some of the output variables should be the lick direction (per trial, left vs right), behavioral context (per trial, whether there was an auditory stimulus or not), the outcome (per trial, correct vs not). 
- Some of the input variables should be the kinematic behavior features (continuous, time-dependent).
