...
save_folder = '/scratch/users/kurgyis/data/map_r2/embed_timeshift_fixed_folds/'
preprocessed_ephys_data_folder = '/oak/stanford/groups/shauld/kurgyis/data/Map_ULTIMATE_preprocessed/stride3_bw40/'
trial_fold_inds_folder = '/oak/stanford/groups/shauld/kurgyis/data/MAP_cv_folds/'
...
window_size = 5
kfold = 5
timeshifts = np.arange(-30,31,2,dtype = int)
...
for timeshift in timeshifts:
    ...
    for embed_file in [embed_files[run_number]]:
        ...
        embed_trial_inds = np.array(embed_data['trial_inds']) - 1

        session_string = embed_file.split('/')[-1].split('_aligned')[0]
        fold_filename = session_string + '_cv_fold_inds.pickle'
        ...
        n_trials = len(fold_data['test_0']) + len(fold_data['train_0'])
        Ntime, _, n = embed_aligned.shape
        ...
        y_test = np.zeros((calc_Ntime,n_trials,fr_aligned.shape[2]))
        y_pred = np.zeros((calc_Ntime,n_trials,fr_aligned.shape[2]))
        ...
        for t in range(0,Ntime, every_nth_timepoint):
            ...
            X = np.concatenate(embed_aligned[start:end], axis = 1) # Reshape to [trials, features]
            ...
            r2_scores[it,:], y_test[it,:], y_pred[it], trial_type_masks[:], test_trial_inds[:] = \
                func.cross_validation_with_predefined_folds(model, X, y, stratification_mask, fold_data, embed_trial_inds, cv = kfold)
